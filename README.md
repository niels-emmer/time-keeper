> READ THIS FIRST: This entire project was generated by Claude Code (Sonnet 4.6), guided by some wicked prompting, all kinds of 'ah, and can you also...' and a bit of debugging. Kudos to Claude for speed, UI and overall architecture. And for displaying emergent behavior such as adding itself as a contributor to the repo ;-) The (generated) docs however, do not mention "security" very often, which is not by accident. This code uses unvetted libraries, has not been pentested and might secretly steal your toothbrush for all I know. Refer to [SECURITY.md](SECURITY.md) for the nitty gritty.
