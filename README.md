> [!CAUTION]
> This entire project was generated by Claude Code (Sonnet 4.6), guided by pointed human prompting, a steady stream of “ah, and can you also…”, and two evenings of iterative back and forth. Started from an empty folder, it evolved under its own self-authored guidance system in /docs; the only human-written content here is this opening paragraph. Credit where it is due: impressive speed, clean UI, and a solid overall architecture. It even keeps its documentation up-to-date unprompted, in both the repository and the Wiki.
> However, you may notice the generated documentation rarely mentions “security” which is intentional. The code relies on unvetted libraries, has not been pentested, and could theoretically exfiltrate your toothbrush for all I know. See [SECURITY.md](SECURITY.md) for the details.
